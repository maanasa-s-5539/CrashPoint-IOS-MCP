...
## Why This Server Cannot Fetch Apptics Crashes Itself

The `crashpoint-ios` MCP server has **no Apptics credentials** and is explicitly **forbidden from calling the Apptics MCP directly** (see `AGENTS.md` — "No direct access from this server: CrashPoint-IOS-MCP must **never** make HTTP requests or tool calls to the Apptics MCP"). That is why this prompt orchestrates the Apptics fetch on the client side:
...
5. For each crash entry from step 3, process ONE AT A TIME in sequence:
   ...
   b. **[MANDATORY — do NOT skip for version-matched crashes]** Call `ZohoApptics_getCrashSummaryWithUniqueMessageId` via the {{APPTICS_MCP_NAME}} MCP server for that single crash's UniqueMessageID to retrieve the full crash detail including the `Message` field (complete crash report text with stack trace). **Step 5b is MANDATORY before 5c for version-matched crashes — you cannot skip fetching the crash detail.** If getCrashSummaryWithUniqueMessageId returns no Message, wait 3 seconds and retry up to 2 times before skipping.
