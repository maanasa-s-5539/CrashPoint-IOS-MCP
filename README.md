...
Once both the CrashPoint iOS MCP and the Zoho Projects MCP are connected to your MCP host (Claude Desktop or Cursor), the host can invoke the `report_to_zoho_projects` tool to create bugs in Zoho Projects for analyzed crash groups. The MCP host handles the authenticated Zoho Projects connection — no additional credentials are required in CrashPoint.
...
Use the `set_fix_status` MCP tool to mark a crash signature as fixed or unfixed.

---

## AI Analysis

AI-powered analysis (pattern insights, root cause suggestions, priority ranking) is a **separate manual step** — it is not part of the automated scheduled pipeline. Use Claude Desktop or Cursor with the MCP tools connected for interactive analysis.
