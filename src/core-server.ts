...
server.registerTool(
  ...
  {
    description:
      "Fetch crash details from the Apptics API in parallel and write .crash files server-side. More efficient than calling getCrashSummaryWithUniqueMessageId through the MCP client one-by-one.",
    inputSchema: z.object({
      crashes: z.array(z.object({
        UniqueMessageID: z.string().describe("Unique crash identifier from Apptics"),
        Exception: z.string().optional().describe("Exception type string"),
